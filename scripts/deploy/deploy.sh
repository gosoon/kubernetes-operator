...
master() {
    ...
    [ -f ${DEPLOY_MASTER_LOG} ] && { echo "master is deployed"; exit 1; } || touch ${DEPLOY_MASTER_LOG}
    ...
    [ -f deploy_coredns.sh ] && bash -x deploy_coredns.sh | tee -a ${DEPLOY_MASTER_LOG} || exit 1
    [ -f deploy_calico.sh ] && bash -x deploy_calico.sh | tee -a ${DEPLOY_MASTER_LOG} || exit 1
}

node() {
    echo -n "starting deploy node... "
    [ -f ${DEPLOY_NODE_LOG} ] && { echo "node is deployed"; exit 1; } || touch ${DEPLOY_NODE_LOG}
    [ -f base_env_01.sh ] && bash -x base_env_01.sh | tee -a ${DEPLOY_NODE_LOG} || exit 1
    [ -f deploy_docker.sh ] && bash -x deploy_docker.sh | tee -a ${DEPLOY_NODE_LOG} || exit 1
...
etcd() {
    ...
    [ -f ${DEPLOY_ETCD_LOG} ] && { echo "etcd is deployed"; exit 1; } || touch ${DEPLOY_ETCD_LOG}
